        ...
        <% try{                          
			...
			+"<a href='#' class='js-navmain-nav-toggle navmain-nav-toggle' data-toggle='collapse' data-target='#navbar' aria-expanded='false' aria-controls='navbar'><i></i></a>"
		        ...
		        +"</ul>	</div> </nav></div></header>"
                     ...
                     out.println("<form action='AgregarAlimentoAdminServlet' method='POST'>"
                             + "<input type='hidden' value='"+Nickname+"' name='Nickname'>"
                             + "<input type='submit' value='Agregar Alimento'>"
                             + "</form>");
                     //MostraTodos los Alimento
                             out.println("<table border='1'>"
                                     ...
                                     + "<td></td>"
                                     + "</tr>");
                             List<Alimento> listaAlimento =  AlimentoDao.getAllAlimentos();
                             for(Alimento lAlimento:listaAlimento){
                                 out.println("<tr>"
                                 + "<td></td>"
                                 + "<td>"+lAlimento.getNombreAlimento()+"</td>"
                                 + "<form action='EditarAlimentoAdminServlet' method='POST'>"
                                 + "<td>"
                                 + "<input type='hidden' value='"+lAlimento.getNombreAlimento()+"' name='AlimetoElegido'>"
                                 ...
                                 + "</form>");
                                     //se despliega boton de borrar
                                      out.println(" "
                                         + "<form action='BorrarAlimentoServlet' method='POST'>"
                                         + "<td>"
                                         + "<input type='hidden' value='"+lAlimento.getNombreAlimento()+"' name='AlimentoElegido'>"
                                         + "<input type='hidden' value='"+u.getNickname()+"' name='Nickname'>"
